...
PGconn * GetConnection(bool replication){
	PGconn	   *tmpconn;
...
bool Connection::RunIdentifySystem(char **sysid, TimeLineID *starttli,
                                   XLogRecPtr *startpos, char **db_name){
    PGconn *conn = this->conn;
	PGresult   *res;
	uint32		hi;
    uint32 lo;
...
bool Connection::CreateReplicationSlot(const char *slot_name, const char *plugin,
...
bool Connection::DropReplicationSlot(const char *slot_name){
    PGconn *conn = this->conn;
	PGresult   *res;

	Assert(slot_name != NULL);

	auto query = createPQExpBuffer();
